...
export default function App() {
  ...
  // Maps a Postgres "days" row into the shape the rest of the app expects
  const rowToDayData = (row: any): DayData => ({
    id: row.date,
    vormittag: row.vormittag,
    nachmittag: row.nachmittag,
    reserve: row.reserve,
    calculatedNewReserve: row.calculated_new_reserve,
    workSumMinutes: row.work_sum_minutes,
    daySaldoMinutes: row.day_saldo_minutes,
  ...
  useEffect(() => {
    ...
    const fetchDays = async () => {
      ...
      setOverviewDays((data || []).map(rowToDayData));
      setOverviewLoading(false);
    };
  ...
  // Execute direct save to Supabase
  const executeDirectSave = async (k: string, payload: any) => {
    if (!user) return;
    try {
      const { error } = await supabase
        .from('days')
        .upsert({ user_id: user.id, date: k, ...payload }, { onConflict: 'user_id,date' });
      if (error) throw error;
      setSyncStatus('Synchronisiert');
    } catch (err: any) {
  ...
  const scheduleDailySave = (
    ...
    const payload = {
      ...
      calculated_new_reserve: newReserve,
      work_sum_minutes: workSum,
      day_saldo_minutes: daySaldo,
  ...
  const handleResetDay = async () => {
    ...
    setReserve('+00:00');
    const k = dateKey(currentDateObj);
    await supabase.from('days').delete().eq('user_id', user.id).eq('date', k);
      ...
      const { error } = await supabase.from('days').upsert(
        {
          ...
          work_sum_minutes: ovTotalWorkMins,
          target_minutes: ovTargetMins,
          day_saldo_minutes: ovSaldoMins,
          reserve: ovReserve.trim() || '+00:00',
          calculated_new_reserve: calculatedNewReserve,
          updated_at: new Date().toISOString(),
        },
    ...
    try {
      if (authMode === 'login') {
        ...
        const { error } = await supabase.auth.signUp({
          email,
          password: loginPassword,
          options: { data: { display_name: loginName } },
        });
        if (error) throw error;
      }
    } catch (err: any) {
    ...
    <div className="container" id="app-screen">
      ...
      <div className="topbar">
        ...
        <div className="user-badge">
          ...
          <button className="btn-logout" id="logout-btn" onClick={() => supabase.auth.signOut()}>
            Abmelden
          </button>
        </div>
      </div>
